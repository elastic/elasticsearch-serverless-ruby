...
Removed these APIs not available on Serverless:
* `enrich.stats`
* `indices.delete_data_lifecycle`

Changes in `scroll` APIs, the parameter `scroll_id` has been removed as it's been deprecated, should be sent via the body.
* `clear_scroll`
* `scroll`
